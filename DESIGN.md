...
Culling alone stopped working at 32+ cars. **Compress** — a toolbar toggle beside All rounds
(`c`), independent of it and off by default so the tree is otherwise drawn as it always was —
applies four rules, each measured at 1080p with a 35-car field; together they took the auto
view from a third scale to full:
...
by slot. Every dependency is satisfied by construction, and the display lays columns out in
this order.
