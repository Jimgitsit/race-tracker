...
- Live roster list: photo, name, joined-at. **Remove** (mis-entries, duplicates) behind a
  ...
  Tapping the photo (or placeholder) opens "<name>'s photo" with it large and two actions:
...
Culling alone stopped working at 32+ cars, and four more rules got the view back to
readable (each measured at 1080p with a 35-car field; together they took it from a third
scale to full). The first and last apply always; the two in the middle are the **Compress**
toggle, which sits beside All rounds in the toolbar (`d` cycles auto → Compress → All):
...
- **A round taller than 8 rows folds** (Compress): two heats to a row, interleaved so
...
- **Consecutive collapsed rounds stack** (Compress) in one narrow column instead of each taking a column
